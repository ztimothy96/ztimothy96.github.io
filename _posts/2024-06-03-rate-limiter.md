...
---
I have found the motivation to become an excellent software engineer, which means I will probably get there pretty soon. (I have a track record of becoming a top 5% performer in basically every activity I have ever worked hard at.) The motive itself I will keep a secret, but let's just say it is not something that anyone else has ever suggested to me. To those of you who believed in me, thank you. To those of you who didn't: meh, you were wrong.
...
We usually want to put the rate limiter on the servers or in middleware. For obvious reasons, large companies with an army of engineers and giant budgets prefer to build custom rate limiters, while smaller companies go with middleware options.

### Algorithms
I'm not trying to learn algorithms right now; I'm trying to focus on system design, so I'll satisfy myself with only a brief overview of rate-limiting approaches.
...
All of these approaches have the same basic idea: create some limit for how many requests we can fulfill and a counter for how many requests we've received. Throttle all the requests that go over the limit. To store the counter, a cache (like Redis) is ideal because it doesn't require too much memory and it can keep latency low.

## Detailed design
A basic rate limiter is pretty simple to design: store the rate-limiting rules and the counter in some caches. Users send requests to middleware, which fetches rules and counters and decides whether to throttle. If the request goes through, we feed it to the API servers. If it is throttled, we can drop the request or store it in a queue for processing later, and we return an HTTP error code to the client.
...
Using middleware helped us to fulfill functional requirements, and we fulfilled non-functional requirements by picking appropriate kinds of components.
- Fault tolerance: achieved by replicating the rate-limiting servers.
- Low latency: achieved by keeping rules and counters in a cache. 
- Low memory: achieved by picking a simpler rate-limiting algorithm that does not require us to look up timestamps, and by ejecting counts from the cache after a fixed time window expires.

Not much to say here, except maybe to add ways that we can write clients to avoid sending excessive requests. Other than that, maybe I should go back and look in detail at the solutions to race conditions and synchronization implemented in Redis...
