...
For instance, we might want to know whether our ship is sailing smoothly or making sharp turns. Since we don't really know how to differentiate on a curved surface, we'll just say that the ship's trajectory is smooth if it looks smooth on our chart. Formally, we're given a curve $$\gamma: \mathbb{R} \to U \subseteq M$$, and we pick a chart $$x: U \to \mathbb{R}^d$$. We'd like to say that $$\gamma$$ is *smooth* (or *$$C^{\infty}$$*, or *infinitely differentible*) if $$x \circ \gamma$$ is. Unfortunately, the smoothness of $$\gamma$$ really depends on which chart $$(U, x)$$ we picked. To make sure we always get the same answer, we need more information about which parts of the manifold are "smooth" and which are "pointy." We need to require that the charts are compatible with each other.
