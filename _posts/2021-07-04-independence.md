...
Let's begin with the last two questions are, which they ask about events which only depend on what the monkey types out *eventually*. Even if we tossed out the first $$100$$ variables, we can still observe whether the events happen. Recall that a $$\sigma$$-field is a collection of events whose probability we can measure; we want to collect only those events that depend on the tail of the sequence.
...
\[
    \lim_{n \to \infty} \prod_{k \geq n} (1 - \Pr(E_k))
    \leq \lim_{n \to \infty} \exp \left(-\sum_{k = n} \Pr(E_k) \right)
    \leq \exp (-\infty)
    = 0. \; \square
\]
